...
int recfast_dx_H_dz(struct thermo* pth, struct thermorecfast * pre, double x_H, double x, double nH,
  ...
  Tmat = Tmat/(pre->fsR*pre->fsR*pre->meR);
  Trad = Trad/(pre->fsR*pre->fsR*pre->meR);
  ...
  A2s1sH = _Lambda_*pow(pre->fsR, 8)*pre->meR;
...
int recfast_dx_He_dz(struct thermo* pth, struct thermorecfast * pre, double x_He, double x, double x_H, double nH,
  ...
  if (x_He < 1.e-15){
    *dxHe_dz=0.;
    ...
    A2s1sHe = _Lambda_He_*pow(pre->fsR, 8)*pre->meR;
    K_He *= 1./pow(pre->fsR*pre->fsR*pre->meR,3);
