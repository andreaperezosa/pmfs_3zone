...
int background_functions(
  ...
  pvecback[pba->index_bg_H] = pba->pvecvc[pba->index_vc_G]*sqrt(rho_tot-pba->K/a/a);

  /** - compute derivative of H with respect to conformal time */
  pvecback[pba->index_bg_H_prime] = - (3./2.) * pba->pvecvc[pba->index_vc_G] * (rho_tot + p_tot) * a + pba->K/a;
...
int background_indices(
  ...
  int index_bi;
  /* a running index for the vector of varying constants */
  int index_vc;
  ...
  class_define_index(pba->index_vc_alpha,_TRUE_,index_vc,1);
  class_define_index(pba->index_vc_G,_TRUE_,index_vc,1);
  class_define_index(pba->index_vc_me,_TRUE_,index_vc,1);
  pba->vc_size = index_vc;
...
int background_solve(
  ...
  class_alloc(pba->pvecvc,pba->vc_size*sizeof(double),pba->error_message);
...
int background_varconst_of_z(
  ...
  if(z>50. && pba->has_varying_const){
    pba->pvecvc[pba->index_vc_alpha] = pba->varying_alpha;
    pba->pvecvc[pba->index_vc_me] = pba->varying_me;
    pba->pvecvc[pba->index_vc_G] = pba->varying_G;
  }
  else{
    pba->pvecvc[pba->index_vc_alpha] = 1;
    pba->pvecvc[pba->index_vc_me] = 1;
    pba->pvecvc[pba->index_vc_G] = 1;
  }
  return _SUCCESS_;
}
