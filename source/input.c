...
int input_read_parameters_general(struct file_content * pfc,
  ...
  /* Read */
  class_read_flag("varying_fundamental_constants",pba->has_varying_const);
  if(pba->has_varying_const){
    class_read_double("varying_alpha",pba->varying_alpha);
    class_read_double("varying_me",pba->varying_me);
    class_read_double("varying_G",pba->varying_G);
  }
  ...
  /** 10) Multizone model for PMFs */
  class_read_flag("is_multizone",pth->is_multizone);
  if (pth->is_multizone){
    class_read_double("delta_nH_1", pth->delta_nH_1);
    class_read_double("delta_nH_2", pth->delta_nH_2);
    class_read_double("f_input", pth->f_input);
    class_read_double("b_clumping", pth->b_clumping);
  }
...
int input_default_params(struct background *pba,
  ...
  /** 10) Multizone for PMFs */
  pth->is_multizone = _FALSE_;
  pth->delta_SIunit_nH0 = 1;
  pth->delta_nH_1 = 0;
  pth->delta_nH_2 = 0;
  pth->f_input = 0;
  pth->b_clumping = 0; 
