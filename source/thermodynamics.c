...
int thermodynamics_at_z(
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    class_call(background_varconst_of_z(pba,z),
               pba->error_message,
               pth->error_message);
    ...
    sigmaTrescale = pba->pvecvc[pba->index_vc_alpha]*pba->pvecvc[pba->index_vc_alpha]/pba->pvecvc[pba->index_vc_me]/pba->pvecvc[pba->index_vc_me];
...
int thermodynamics_init(
  ...
  /** - check if there is multizone */

  if (pth->is_multizone == _TRUE_ ){
    class_call(thermodynamics_solve_multizone(ppr, pba, pth, ptw),
               pth->error_message,
               pth->error_message);
...
/**
 * The goal of this function is to run thermo_init N+2 times
 * in order to reproduce the model of 2004.09487
 */
int thermodynamics_solve_multizone( 
                                   struct precision * ppr,
                                   struct background * pba,
                                   struct thermo * pth,
                                   struct thermo_workspace * ptw
  ...
  /* structures for storing temporarily information */
  struct thermo th1;
  struct thermo th2;
  struct thermo th3;
  struct thermo * pth1=&th1;
  struct thermo * pth2=&th2;
  struct thermo * pth3=&th3;
  int i=0;
  float sum=0, mult=0;

  /* copy all information on thermo into the tempo thermos */
  memcpy(pth1, pth, sizeof(struct thermo));
  memcpy(pth2, pth, sizeof(struct thermo));
  memcpy(pth3, pth, sizeof(struct thermo));

  pth1->is_multizone = _FALSE_;
  pth2->is_multizone = _FALSE_;
  pth3->is_multizone = _FALSE_;

  /*Get the value of f_v and delta_nH0 */
  double * f_v;
  double * delta_nH;
  class_alloc(f_v, 3*sizeof(double), pth->error_message);
  class_alloc(delta_nH, 3*sizeof(double), pth->error_message);
  f_v[1] = pth->f_input;
  delta_nH[0] = pth->delta_nH_1;
  delta_nH[1] = pth->delta_nH_2;
  f_v[0] = (pth->b_clumping - f_v[1] - pth->b_clumping*f_v[1] + 2*delta_nH[1]*f_v[1] - delta_nH[1]*delta_nH[1]*f_v[1])
           /(1 + pth->b_clumping - 2*delta_nH[0] + delta_nH[0]*delta_nH[0] - delta_nH[0]*delta_nH[0]*f_v[1] + 2*delta_nH[0]*delta_nH[1]*f_v[1] - delta_nH[1]*delta_nH[1]*f_v[1]);
  f_v[2] = 1-f_v[0]-f_v[1];
  delta_nH[2] = (1-f_v[0]*delta_nH[0]-f_v[1]*delta_nH[1])/f_v[2];

  pth1->delta_SIunit_nH0 = delta_nH[0];
  pth2->delta_SIunit_nH0 = delta_nH[1];
  pth3->delta_SIunit_nH0 = delta_nH[2];

 /* printf(z_reio, tau_reio)*/
  /* Initiate the module three times, one for each structure */
  class_call(thermodynamics_init(ppr, pba, pth1),
             pth1->error_message,
             pth->error_message);

  class_call(thermodynamics_init(ppr, pba, pth2),
             pth2->error_message,
             pth->error_message);

  class_call(thermodynamics_init(ppr, pba, pth3),
             pth3->error_message,
             pth->error_message);

  /* Extract the resulting weighted average of ionization fraction */
  int index_z=0, j=0;
  double average;
  double * xe;
  class_alloc(xe, 3*sizeof(double), pth->error_message);
  xe[0]=xe[1]=xe[2]=0.0;
  int * indices;
  class_alloc(indices, 6*sizeof(int), pth->error_message);
  indices[0] = pth->index_th_xe;
  indices[1] = pth->index_th_Tb;
  indices[2] = pth->index_th_dTb;
  indices[3] = pth->index_th_wb;
  indices[4] = pth->index_th_cb2;
  indices[5] = pth->index_th_dkappa;

  for (j = 0; j < 6; j++){
    for (index_z = 0; index_z < pth->tt_size; index_z++){
      average = 0;
      xe[0] = pth1->thermodynamics_table[index_z*pth1->th_size+indices[j]];
      xe[1] = pth2->thermodynamics_table[index_z*pth2->th_size+indices[j]];
      xe[2] = pth3->thermodynamics_table[index_z*pth3->th_size+indices[j]];
      if (j==0){
        for(i = 0; i < 3; i++){
          average += xe[i]*f_v[i]*delta_nH[i];
        }
      }
      else{
        for(i = 0; i < 3; i++){
      ...
      pth->thermodynamics_table[index_z*pth->th_size+indices[j]] = average;
    }
  }

  /*fwrite() f.open -->look this up*/
  FILE * fTH1;
  fTH1 = fopen("thermo1.txt", "w+");
  fprintf(fTH1,"%20s%20s%20s%20s%20s%20s%20s%20s\n", "xe1", "xe2", "xe3" ,"z1","z2","z3", "average_ze", "average_z");
  for(index_z = 0; index_z < pth->tt_size; index_z++){
    fprintf(fTH1, "%20e%20e%20e%20e%20e%20e%20e%20e\n", pth1->thermodynamics_table[index_z*pth1->th_size+pth->index_th_xe], pth2->thermodynamics_table[index_z*pth2->th_size+pth->index_th_xe], pth3->thermodynamics_table[index_z*pth3->th_size+pth->index_th_xe],
                                                        pth1->z_table[index_z],  pth2->z_table[index_z],  pth3->z_table[index_z], 
                                                        pth->thermodynamics_table[index_z*pth->th_size+pth->index_th_xe], pth->z_table[index_z]);
  }
  fclose(fTH1);

  class_call(thermodynamics_free(pth1),
             pth1->error_message,
             pth->error_message);

  class_call(thermodynamics_free(pth2),
             pth2->error_message,
             pth->error_message);

  class_call(thermodynamics_free(pth3),
             pth3->error_message,
             pth->error_message);
  ...
  if (pth->reio_z_or_tau == reio_z) {
    ...
    class_stop(pth->error_message, "In the multizone model the shooting for tau_reio is not implemented");
  }
...
int thermodynamics_helium_from_bbn(
  ...
  */
  ...
  if(pba->has_varying_const){
    class_call(background_varconst_of_z(pba,z_bbn),
             pba->error_message,
             pba->error_message);
    pth->YHe*=1.01*pba->pvecvc[pba->index_vc_alpha];
  }
...
int thermodynamics_workspace_init(
  ...
  /* H number density today in SI units
     delta_SIunit corresponds to the description in 2004.09487. 
     Default value is 1 if not indicated otherwise */
  ptw->SIunit_nH0 = 3.*ptw->SIunit_H0*ptw->SIunit_H0*pba->Omega0_b/(8.*_PI_*_G_*_m_H_)*(1.-ptw->YHe)*pth->delta_SIunit_nH0;
  ...
  class_define_index(ptw->
ptdw->index_ap_He1f,_TRUE_,index_ap,1); // in between 1st and 2nd He recombination
  ...
  for(index_ap=0;index_ap<ptw->ptdw->ap_size;++index_ap){
    class_call(background_varconst_of_z(pba,ptw->ptdw->ap_z_limits[index_ap]),
               pba->error_message,
               pth->error_message);
    ptw->ptdw->ap_z_limits[index_ap]*=pba->pvecvc[pba->index_vc_me]*pba->pvecvc[pba->index_vc_alpha]*pba->pvecvc[pba->index_vc_alpha];
  }
...
int thermodynamics_derivs(
  ...
  /* Shorthand notations */
  double z,x,nH,Trad,Tmat,x_H,x_He,Hz,eps,depsdlna,dHdlna,heat_capacity,rate_gamma_b,alpha,me;
  ...
  class_call(background_at_z(pba,
             ...
             pba->error_message,
             error_message);
  alpha = pba->pvecvc[pba->index_vc_alpha];
  me = pba->pvecvc[pba->index_vc_me];
  ...
  */
  ...
  rate_gamma_b = rescale_rate*( 2. * _sigma_/_m_e_/_c_ ) * ( 4./3. * pvecback[pba->index_bg_rho_g] * _Jm3_over_Mpc2_ ) * x / (1.+x+ptw->fHe);
  /* Heat capacity of the IGM */
...
int thermodynamics_sources(
  ...
  /** Define local variables */
  /* Shorthand notations */
  double z,x=0.,Tmat,Trad,dTmat,sigmaTrescale;
  /* Recfast smoothing */
  ...
  pth->thermodynamics_table[(pth->tt_size-index_z-1)*pth->th_size+pth->index_th_dkappa]
    = (1.+z) * (1.+z) * ptw->SIunit_nH0 * x * _sigma_ * sigmaTrescale * _Mpc_over_m_;
...
int thermodynamics_ionization_fractions(
  ...
  /* Varying fundamental constants */
  double rescale_rhs,rescale_T;
  double fsR = pba->pvecvc[pba->index_vc_alpha];
  double meR = pba->pvecvc[pba->index_vc_me];
  rescale_rhs = fsR*fsR*fsR*meR*meR*meR;
  rescale_T = 1./fsR/fsR/meR;
  ...
  if (current_ap == ptdw->index_ap_brec) {
    ...
    rhs = ptw->SIunit_nH0/exp( 1.5*log(ptw->const_NR_numberdens*Tmat/(1.+z)/(1.+z)) - ptw->const_Tion_HeII/Tmat )/rescale_rhs;
  ...
  /** - --> second regime: first Helium recombination (analytic approximation) */
    ...
    rhs = rescale_rhs * exp( 1.5*log(ptw->const_NR_numberdens*Tmat/(1.+z)/(1.+z)) - ptw->const_Tion_HeII/Tmat ) / ptw->SIunit_nH0;
  ...
  /** - --> third regime: first Helium recombination finished, H and Helium fully ionized */
    ...
    rhs = 0.25*ptw->SIunit_nH0/exp( 1.5*log(ptw->const_NR_numberdens*Tmat/(1.+z)/(1.+z)) - ptw->const_Tion_HeI/Tmat )/rescale_rhs;
  ...
  /** - --> fourth regime: second Helium recombination starts (analytic approximation) */
    ...
    rhs = rescale_rhs * 4.*exp(1.5*log(ptw->const_NR_numberdens*Tmat/(1.+z)/(1.+z)) - ptw->const_Tion_HeI/Tmat ) / ptw->SIunit_nH0;
  ...
  /** - --> fifth regime: Hydrogen recombination starts (analytic approximation)
    ...
    rhs = rescale_rhs * exp(1.5*log(ptw->const_NR_numberdens*Tmat/(1.+z)/(1.+z)) - ptw->const_Tion_H/Tmat)/ptw->SIunit_nH0;
