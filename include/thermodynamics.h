...
struct thermo
{
  ...
  short is_multizone;   /** checks if using multizone model**/
  double f_input;       /** density parameters for multizone model intrduced manually**/
  double delta_nH_1;    /** volume fraction for the multizone model **/
  double delta_nH_2;
  double delta_SIunit_nH0;  /** density parameters used in each multizone model **/
  double b_clumping;    /** clumping factor **/
  //@}
...
extern "C" {
  ...
  int thermodynamics_solve_multizone(struct precision * ppr,
                                    struct background * pba,
                                    struct thermo * pth,
                                    struct thermo_workspace * ptw);
